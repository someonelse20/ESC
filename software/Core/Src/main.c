...
static double get_rotor_angle(double rotor_bemf[3]);
static double get_rotor_speed(double rotor_bemf[3]);
...
int main(void)
{
	...
	while (1)
	{
		...
		rotor_angle = get_rotor_angle(rotor_bemf);
		rotor_speed = get_rotor_speed(rotor_bemf);
...
static void get_rotor_bemf(double *rotor_bemf_p) {
	// Phase A is floating
	if (sequence[step][0] == 0) {
		rotor_bemf_p[0] = process_adc(adc_buf[BEMF_A_INDEX]);
		return;
	}

	// Phase B is floating
	if (sequence[step][1] == 0) {
		rotor_bemf_p[1] = process_adc(adc_buf[BEMF_B_INDEX]);
		return;
	}

	// Phase C is floating
	if (sequence[step][2] == 0) {
		rotor_bemf_p[2] = process_adc(adc_buf[BEMF_C_INDEX]);
		return;
	}
}
